...
export function SignUpForm() {
  ...
  const activeStyle = "bg-white text-main-green border border-main-green shadow-xl scale-105"
  ...
  async function onSubmit(values) {
    ...
    if (response?.code === 200) {
      ...
      const encodedCode = encodeURIComponent(response?.data?.data?.verificationCode)
  ...
  return (
    <div >
      ...
      <Form {...form}>
        <form dir={locale === "ar" ? "rtl" : "ltr"} onSubmit={form.handleSubmit(onSubmit)} className="lg:p-10  p-8 border border-main-gray rounded-lg flex max-lg:flex-col items-start gap-8 w-full ">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormControl>
                  <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className="flex lg:flex-col gap-1">
                    <div className="flex items-center">
                      <RadioGroupItem value="seeker" id="seeker" className="sr-only" />
                      <label
                        dir={locale === "ar" ? "rtl" : "ltr"}
                        htmlFor="seeker"
                        className={`w-30 rounded-s-lg shadow p-3 text-sm font-semibold cursor-pointer ${field.value === "seeker" ? activeStyle : "bg-main-light-green text-main-navy"}`}
                      >
                        {t('seeker')}
                      </label>
                    </div>
                    <div className="flex items-center">
                      ...
                      <label
                        dir={locale === "ar" ? "rtl" : "ltr"}
                        htmlFor="owner"
                        className={`w-30 rounded-s-lg shadow p-3 text-sm font-semibold cursor-pointer ${field.value === "owner" ? activeStyle : "bg-main-light-green text-main-navy"}`}
                      >
                        {t('owner')}
                      </label>
                    </div>
          ...
          <div className="space-y-6 shhrink-0 w-full">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder={t('placeholder')} {...field} className={inputStyle} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input type="email" placeholder={t('placeholder')} {...field} className={inputStyle} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <div className="w-full flex items-center justify-between">
              <Button disabled={isSubmitting} type="submit" className='rounded-none h-12   bg-main-green text-white lg:py-4 lg:!px-8 p-3 rounded-tr-2xl max-lg:text-xs  font-semibold flex items-center gap-2 w-fit '>
                {isSubmitting ? <Loader2 size={20} className="animate-spin" /> : <FaLongArrowAltRight size={20} />}
                <p>{t('submit_button')}</p>
              </Button>
